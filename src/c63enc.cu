...
yuv_t *d_curframe_orig,
    *d_refframe_recons,    // Computing from this (copied from host per frame)
    *d_curframe_predicted; // Computing into this and copying back
yuv_buf d_curframe_origbuf, d_refframe_reconsbuf, d_curframe_predictedbuf;
...
void init_device_state(struct c63_common *cm) {
  ...
  cudaMemcpyErr((uint8_t *)d_cm + offsetof(struct c63_common, refframe),
                &d_refframe, sizeof(d_refframe), cudaMemcpyHostToDevice);
  cudaMemcpyErr((uint8_t *)d_cm + offsetof(struct c63_common, curframe),
                &d_curframe, sizeof(d_curframe), cudaMemcpyHostToDevice);

  cudaMemcpyErr((uint8_t *)d_curframe + offsetof(struct frame, orig),
                &d_curframe_orig, sizeof(d_curframe_orig),
                cudaMemcpyHostToDevice);
  cudaMemcpyErr((uint8_t *)d_refframe + offsetof(struct frame, recons),
                &d_refframe_recons, sizeof(d_refframe_recons),
                cudaMemcpyHostToDevice);
  cudaMemcpyErr((uint8_t *)d_curframe + offsetof(struct frame, predicted),
                &d_curframe_predicted, sizeof(d_curframe_predicted),
                cudaMemcpyHostToDevice);
  ...
  cudaMemcpyErr(d_curframe_orig, &curframe_orig_yuv, sizeof(struct yuv),
                cudaMemcpyHostToDevice);
  cudaMemcpyErr(d_refframe_recons, &refframe_recons_yuv, sizeof(struct yuv),
                cudaMemcpyHostToDevice);
  cudaMemcpyErr(d_curframe_predicted, &curframe_predicted_yuv,
                sizeof(struct yuv), cudaMemcpyHostToDevice);

  cudaMemcpyErr((uint8_t *)d_curframe + offsetof(struct frame, mbs),
