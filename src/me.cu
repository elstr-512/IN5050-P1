...
extern yuv_buf *d_curframe_origbuf, *d_refframe_reconsbuf,
...
__device__ static void sad_block_8x8(uint8_t *block1, uint8_t *block2,
  ...
  int shuffled_result = abs(block2[stride * threadIdx.y + threadIdx.x] -
                            block1[stride * threadIdx.y + threadIdx.x]);

  for (int offset = 16; offset > 0; offset /= 2) {
    shuffled_result +=
        __shfl_down_sync(SHUFFLE_FULL_MASK, shuffled_result, offset);
  }
...
__device__ static void me_block_8x8(struct c63_common *cm, int mb_x, int mb_y,
                                    uint8_t *orig, uint8_t *ref,
                                    int color_component) {
  int tid = threadIdx.x + threadIdx.y * blockDim.x;
...
__device__ void c63_motion_estimate_gpu(struct c63_common *cm) {
  ...
  if (color_component == 0) {
    // Y component
    me_block_8x8(cm, mb_x, mb_y, cm->curframe->orig->Y, cm->refframe->recons->Y,
                 Y_COMPONENT);
...
void c63_estimate_compensate(struct c63_common *cm) {
  ...
  cudaMemcpyErr(d_curframe_origbuf, cm->curframe->orig->buf,
                cm->total_yuv_buflen, cudaMemcpyHostToDevice);

  cudaMemcpyErr(d_refframe_reconsbuf, cm->refframe->recons->buf,
                cm->total_yuv_buflen, cudaMemcpyHostToDevice);
  ...
  cudaMemcpyErr(cm->curframe->predicted->buf, d_curframe_predictedbuf,
                cm->total_yuv_buflen, cudaMemcpyDeviceToHost);

  /* Copy back current frame macroblocks (Y, U and V) */
  cudaMemcpyErr(cm->curframe->mbs[Y_COMPONENT], d_curframe_mby, cm->y_mb_buflen,
                cudaMemcpyDeviceToHost);

  cudaMemcpyErr(cm->curframe->mbs[U_COMPONENT], d_curframe_mbu, cm->u_mb_buflen,
                cudaMemcpyDeviceToHost);

  cudaMemcpyErr(cm->curframe->mbs[V_COMPONENT], d_curframe_mbv, cm->v_mb_buflen,
                cudaMemcpyDeviceToHost);
}
#else
/* ~~~~~~~~~~~~~~~~ ~~~~~~~~~~~~~~~~ ~~~~~~~~~~~~~~~~ */
